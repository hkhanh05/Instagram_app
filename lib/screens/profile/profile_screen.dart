...
class ProfileScreen extends StatefulWidget {
  ...
  const ProfileScreen({super.key, this.currentUserId = 1});
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadSqliteData() async {
    ...
    try {
      final user = await DatabaseHelper.instance.getUserById(widget.currentUserId);
      final posts = await DatabaseHelper.instance.getPostsByUserId(widget.currentUserId);
      final highlightsData = await DatabaseHelper.instance.getHighlights(widget.currentUserId); 
      ...
      if (mounted) {
        setState(() {
          ...
          currentSong = user?['currentSong'] ?? "Thêm nhạc vào trang cá nhân";
          currentUrl = user?['musicUrl'] ?? "";
          _isLoading = false;
        });
      }
    } catch (e) {
      print("❌ Lỗi load dữ liệu DB: $e");
  ...
  void _confirmDeleteHighlight(int highlightId, String title) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text("Bạn có chắc chắn muốn xóa '$title' khỏi trang cá nhân không?"),
        actions: [
          ...
          child: Column(
            ...
            children: [
              ...
              Theme(
                ...
                child: ListTile(title: Text(currentSong, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
              ),
              const Divider(color: Colors.grey),
              ListTile(
                leading: const Icon(Icons.swap_horiz, color: Colors.white),
                title: const Text("Thay đổi bài hát trên trang cá nhân", style: TextStyle(color: Colors.white)),
                onTap: () { Navigator.pop(context); _changeSong(); },
              ),
              ListTile(
              ...
              ListTile(
                ...
                onTap: () {
                  setState(() { currentSong = "Thêm nhạc vào trang cá nhân"; isPlaying = false; currentUrl = ""; });
                  _player.stop();
                  Navigator.pop(context);
                },
              ),
            ...
            return Column(
              children: [
                ...
                const Padding(
                  ...
                  child: Text('Chia sẻ', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                ),
                ...
                Expanded(
                  child: followers == 0
                      ? const Center(child: Text("Không có người theo dõi nào để chia sẻ", style: TextStyle(color: Colors.grey)))
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return ListTile(
                              leading: CircleAvatar(
                                ...
                                backgroundImage: _userData?['avatarUrl'] != null ? NetworkImage(_userData!['avatarUrl']) : null,
                                child: _userData?['avatarUrl'] == null ? const Icon(Icons.person, color: Colors.grey) : null,
                              ),
                              title: Text("friend_insta_$index", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                              subtitle: Text("Người theo dõi thứ $index", style: const TextStyle(color: Colors.grey, fontSize: 12)),
                              trailing: SizedBox(
                                height: 32,
                                width: 80,
                                child: ElevatedButton(
                                  onPressed: () {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                        content: Text("Đã gửi trang cá nhân đến friend_insta_$index thành công!"),
                                  ...
                                  child: const Text("Gửi", style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold)),
                                ),
                              ),
                            );
                          },
  ...
  Widget _buildAppBarTitle() {
    return Row(
      ...
      children: [
        ...
        Text(_userData?['username'] ?? '', style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 18)),
      ...
      IconButton(
        ...
        onPressed: () async {
          ...
          await DatabaseHelper.instance.insertPost(
            ...
            currentAvatar, 
            'Bài viết mới cập nhật tự động từ SQLite! 🚀'
          );
          _loadSqliteData();
        }
      ),
      IconButton(icon: const Icon(Icons.menu, color: Colors.black), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (context) => const SettingsScreen()))),
    ];
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              CircleAvatar(
                ...
                child: avatar.isEmpty ? const Icon(Icons.person, size: 40, color: Colors.grey) : null,
              ),
              ...
              Expanded(
                child: Row(
                  ...
                  children: [
                    ...
                    _StatItem(count: '$followers', label: 'Người theo dõi', onTap: () => _goToFollow(context, 0, followers, following)),
                    _StatItem(count: '$following', label: 'Đang theo dõi', onTap: () => _goToFollow(context, 1, followers, following)),
                  ],
                ),
              ),
            ],
          ...
          const SizedBox(height: 10),
          Text(_userData?['fullName'] ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
          Text(_userData?['bio'] ?? ''),
  ...
  void _goToFollow(BuildContext context, int index, int followers, int following) {
    Navigator.push(context, MaterialPageRoute(builder: (_) => InstagramFollowScreen(
      initialIndex: index, 
                ...
                children: [
                  Container(
                    ...
                    child: CircleAvatar(
                      ...
                      backgroundImage: (!isAddButton && highlightImg.isNotEmpty) ? NetworkImage(highlightImg) : null,
                      child: isAddButton ? const Icon(Icons.add, color: Colors.black, size: 30) : (highlightImg.isEmpty ? const Icon(Icons.person, color: Colors.grey) : null),
                    ),
                  ),
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) => PostDetailScreen(
                  ...
                  onPostDeleted: _loadSqliteData, // Khi xóa bài viết bên trong, tự động load lại danh sách trang chính
                ),
              ),
            );
          },
...
class InstagramFollowScreen extends StatelessWidget {
  ...
  const InstagramFollowScreen({super.key, this.initialIndex = 0, required this.username, required this.followersCount, required this.followingCount});
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(children: [
          FollowListContent(isFollowerTab: true, maxItems: followersCount, currentUsername: username), 
          FollowListContent(isFollowerTab: false, maxItems: followingCount, currentUsername: username)
        ]),
      ),
    );
  }
  ...
  void _confirmDeletePost(int postId) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text("Bài viết này sẽ bị xóa vĩnh viễn khỏi SQLite của bạn."),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: const Text("Hủy", style: TextStyle(color: Colors.black))),
          TextButton(
            onPressed: () async {
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              leading: CircleAvatar(
                                ...
                                child: widget.avatarUrl.isEmpty ? const Icon(Icons.person, size: 15) : null,
                              ),
                      ...
                      TextButton(
                        onPressed: () async {
                          if (commentController.text.isNotEmpty) {
                            await DatabaseHelper.instance.insertComment(postId, widget.username, commentController.text);
    ...
    return Scaffold(
      ...
      body: _currentPosts.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) {
                int actualIndex = (index + widget.initialIndex) % _currentPosts.length;
    ...
    return Column(
      ...
      children: [
        ListTile(
          ...
          title: Text(widget.username, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
        ...
        postImg.isNotEmpty 
            ...
            : Container(height: 300, color: Colors.grey[200], child: const Icon(Icons.broken_image, size: 50, color: Colors.grey)),
        Padding(
              ...
              GestureDetector(
                onTap: () async {
                  setState(() {
                    ...
                    _likesMap[postId] = isLiked ? currentLikes + 1 : currentLikes - 1;
                  });
                  await DatabaseHelper.instance.updatePostLikes(postId, _likesMap[postId]!);
                },
        ...
        Padding(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text('$currentLikes lượt thích', style: const TextStyle(fontWeight: FontWeight.bold)), 
              ...
              GestureDetector(
                onTap: () => _showCommentSheet(postId),
                child: Text('Xem tất cả $commentLength bình luận', style: const TextStyle(color: Colors.grey, fontSize: 14)), 
              ),
...
class FollowListContent extends StatefulWidget {
  ...
  const FollowListContent({super.key, required this.isFollowerTab, required this.maxItems, required this.currentUsername});
...
class _FollowListContentState extends State<FollowListContent> {
    ...
    return ListView(
      children: [
        ...
        if (widget.isFollowerTab && widget.maxItems > 0) ...[
          ...
          _buildCategoryItem("Người không theo dõi lại", "Các tài khoản liên quan đến ${widget.currentUsername}", Icons.group_remove_outlined),
          _buildCategoryItem("Ít tương tác nhất", "Danh sách phân tích tương tác", Icons.unfold_less),
          const Divider(),
        ],
        widget.maxItems == 0 
        ? const Padding(
            padding: EdgeInsets.all(30.0),
            child: Center(child: Text("Danh sách trống", style: TextStyle(color: Colors.grey))),
          )
        : ListView.builder(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
  ...
  Widget _buildListTrailing(bool isFollowerTab, int index) {
    bool isFollowedBack = _followingMap[index] ?? (isFollowerTab && index % 3 == 0);
    return Row(
      mainAxisSize: MainAxisSize.min,
      children: [
        SizedBox(
          height: 30,
          child: ElevatedButton(
            onPressed: () => setState(() => _followingMap[index] = !isFollowedBack),
            style: ElevatedButton.styleFrom(backgroundColor: isFollowedBack ? Colors.blue : Colors.grey[200], elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8))),
            child: Text(isFollowedBack ? "Theo dõi lại" : "Nhắn tin", style: TextStyle(color: isFollowedBack ? Colors.white : Colors.black, fontSize: 12, fontWeight: FontWeight.bold)),
          ),
        ),
        const SizedBox(width: 8),
